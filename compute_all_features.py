...
from feature_utils import smooth_contour, determine_overlap, get_maximum_bounding, area_of_intersection
from clock_digit_priors import get_angle_priors

from datetime import date, timedelta, datetime
from dateutil.parser import parse

def compute_all_features(df, index, image):
    vis = image.copy()
    ...
    cv2.drawContours(bleached, [best_curve], -1, (255, 255, 255), 20)
    black = 255 - bleached
    ...
    for crop in number_crops:
        ...
        r = np.linalg.norm(np.array([center_X, center_Y]) - np.array([box_center_x, box_center_y]))
...
if __name__ == '__main__':
    ...
    print("Found ", len(images), " images to process...")

    for i, image in enumerate(images):
        df, vis = compute_all_features(df, i, images[i])
        cv2.imwrite(save_directory + files[i], vis)
