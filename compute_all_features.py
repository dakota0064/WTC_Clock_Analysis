...
def compute_all_features(df, index, image):
    ...
    model = load_model("data/mnist_threshed_classifier.h5")
...
if __name__ == '__main__':
    ...
    base_directory = "data/cropped_images_full"
    save_directory = "data/annotated_images/"
    if not os.path.isdir(save_directory):
        os.mkdir(save_directory[:-1])

    df = pd.read_csv("data/clock_16000_labels.csv")
    ...
    for i in range(len(df)):
        ...
        df, vis = compute_all_features(df, i, image)
